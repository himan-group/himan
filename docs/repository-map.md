...
- `system setup|doctor|audit stats|list|issues|migrate <path>`; `cleanup` is planned for the `system` group
...
- A private local source lives at `~/.himan/local-source/`; it is created by `himan system migrate` so unmanaged resources can be onboarded without a Git repository. Its source type is `local` and its resources are versioned by `himan.yaml`.
