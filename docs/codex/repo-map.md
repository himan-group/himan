...
- `resource list|history|create|archive|restore|rename`; `rename` is currently marked not recommended; `resource list` without a type groups all active source resources, `--brief` hides descriptions, `--archived` lists archived resources, `--include-archived` includes archived resources, and `--installed` lists current project installs instead of source resources
- `project list|install|dev|uninstall|publish`; direct installs require `--include-archived` for archived resources, while lock-file installs can restore archived resources already recorded in `himan.lock`
