...
- `resource list|history|create|comment|dev|publish|archive|restore|rename`; `rename` is currently marked not recommended; `resource list` without a type groups all active source resources, sorts resources within each category by descending `comment.score` with unrated resources last, `--source` selects a source alias for source lists, `--brief` hides descriptions, `--comment` shows comment text, `--archived` lists archived resources, `--include-archived` includes archived resources, and `--installed` lists current project installs instead of source resources; `resource publish` can use `--source <alias>`
...
- GitHub Actions verify pushes to `dev` and PRs into `dev` or `master` with typecheck, tests, and build; publish to npm through npm Trusted Publishing/OIDC after PRs are merged to `master` or commits are pushed to `master`, skipping npm publish when the package version already exists; check that `v{package.version}` does not already exist for PRs into `master`; and create/push `v{package.version}` tags after PRs are merged to `master` or commits are pushed to `master`.
